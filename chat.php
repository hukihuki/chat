...
window.onload = function(){
  ...
  document.querySelector("#sbmt").addEventListener("click",function(){
      ...
      var request = new XMLHttpRequest();
        request.open('POST', 'http://127.0.0.1/chat3/chat/set.php', false);
...
function auth(){
  var request = new XMLHttpRequest();
  request.open('POST', 'http://127.0.0.1/chat3/chat/auth.php', false);
  request.onreadystatechange = function(){
    if (request.status === 200 || request.status === 304 ) {
      var response = request.responseText;
      var json     = JSON.parse(response);
...
function getLog(){
	var request = new XMLHttpRequest();	
	request.open('GET', 'http://127.0.0.1/chat3/chat/get.php', false);
	request.onreadystatechange = function(){
		if (request.status === 200 || request.status === 304 ) {
			var response = request.responseText;
			var json     = JSON.parse(response);
